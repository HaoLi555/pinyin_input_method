...
def pinyin_to_chn_chars(input_pinyin, trigram, bigram_weight, trigram_weight):
    ...
    for pinyin_sentence in tqdm(input_pinyin, desc="Processing sentences"):
        ...
        for index, words in tqdm(enumerate(words_sequence), desc="Processing words"):
            if index == 0:
                ...
                now_layer = [
                    (words[i], -math.log(frequency_table[words[i]] / TOT_CHAR))
                    if words[i] in frequency_table.keys()
                    else (words[i], -math.log(SMALL_PROBABILITY))
                ...
                for i in words:
                    ...
                    for j in last_layer:
                        ...
                        if not trigram or index == 1:
                            metric = (
                                -math.log(
                                    frequency_table[j[0][-1] + i]
                                    / frequency_table[j[0][-1]]
                                )
                                + j[1]
                                if j[0][-1] + i in frequency_table.keys()
                                else -math.log(SMALL_PROBABILITY) + j[1]
                            )
                        else:
                            if j[0][-2:] + i in trigram_frequency_table.keys():
                                metric = (
                                    -math.log(
                                        (
                                            trigram_frequency_table[j[0][-2:] + i]
                                            / frequency_table[j[0][-2:]]
                                        )
                                        * trigram_weight
                                        + (
                                            frequency_table[j[0][-1] + i]
                            ...
                            elif j[0][-1] + i in frequency_table.keys():
                                metric = (
                                    -math.log(
                                        (
                                            frequency_table[j[0][-1] + i]
                                ...
                                metric = -math.log(SMALL_PROBABILITY) + j[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path", type=str, default="data/output.txt", help="Output file path."
    )
    parser.add_argument(
        "--trigram", action="store_true", help="Whether to use a trigram model."
    )
    parser.add_argument(
        "--frequency_table_path",
        type=str,
        default="src/data/frequency_table.txt",
        help="Path of frequency table file.",
    )
    parser.add_argument(
        "--trigram_frequency_table_path",
        type=str,
        default="src/data/trigram_frequency_table.txt",
        help="Path of trigram frequency table file.",
    )
    parser.add_argument(
        "--bigram_weight",
        type=float,
        default=0.7,
        help="Weight of bigram (0~1) when use trigram model, e.g. bigram_weight=0.7 then trigram_weight=0.3.",
    )
    ...
    assert 0 < args.bigram_weight < 1
    bigram_weight = args.bigram_weight
    trigram_weight = 1 - bigram_weight
    ...
    chn_output = pinyin_to_chn_chars(
        input_pinyin,
        trigram=trigram,
