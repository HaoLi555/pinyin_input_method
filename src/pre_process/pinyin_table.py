...
def build_pinyin_table():
    with open(RAW_PINYIN_TABLE_PATH, "r", encoding="GBK") as f:  # encoding='GBK'
        with open(
            RAW_CHN_CHARS_TABLE_PATH, "r", encoding="GBK"
        ) as ref:  # encoding='GBK'
            ...
            with open(SAVE_PATH, "w") as savef:
                json.dump(pinyin_table, savef, ensure_ascii=False)  # ensure_ascii=False
